...
public class dob extends HttpServlet {
	...
	public void doPost(HttpServletRequest req , HttpServletResponse res) throws ServletException, IOException{
		...
		String d=req.getParameter("dob2");
		...
		try {
			...
			if(rs.next())
			{
					PreparedStatement pst1=conn.prepareStatement("update obs set dob=? where name=? and password=?");
					pst1.setString(1,d);
					pst1.setString(2,name);
					pst1.setString(3,passwrd);
					pst1.executeUpdate();
					
					pw.println("<script>alert('Date of Birth has been changed Successfully !!!!!!'); window.location='login.html'</script>");	
				}
