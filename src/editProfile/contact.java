...
public class contact extends HttpServlet {
	...
	public void doPost(HttpServletRequest req , HttpServletResponse res) throws ServletException, IOException{
		...
		try {
			...
			if(rs.next())
			{
					PreparedStatement pst1=conn.prepareStatement("update obs set contact=? where name=? and password=?");
					pst1.setString(1,con);
					pst1.setString(2,name);
					pst1.setString(3,passwrd);
					pst1.executeUpdate();
					
					pw.println("<script>alert('Contact no. has been changed Successfully !!!!!!'); window.location='login.html'</script>");	
				}
