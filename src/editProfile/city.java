...
public class city extends HttpServlet {
	...
	public void doPost(HttpServletRequest req , HttpServletResponse res) throws ServletException, IOException{
		...
		try {
			...
			if(rs.next())
			{
					PreparedStatement pst1=conn.prepareStatement("update obs set city=? where name=? and password=?");
					pst1.setString(1,c);
					pst1.setString(2,name);
					pst1.setString(3,passwrd);
					pst1.executeUpdate();
					
					pw.println("<script>alert('City has been changed Successfully !!!!!!'); window.location='login.html'</script>");	
				}
